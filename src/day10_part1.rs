...
fn get_answer(input: &str) -> usize {
    ...
    loop {
        ...
        if let Some(solution) = propagate(&mut robots, starting_id.unwrap()) {
            return solution;
        }
    }
    println!("{:#?}", &robots);
...
fn destroy_chip(robots: &mut HashMap<usize, Robot>, chip: usize, bot_from: usize) {
...
fn propagate(robots: &mut HashMap<usize, Robot>, bot_id: usize) -> Option<usize> {
    ...
    if gchip_low == Destination::Bot {
        ...
        destroy_chip(robots, chip_low, bot_id);
    }
    if gchip_high == Destination::Bot {
        ...
        destroy_chip(robots, chip_high, bot_id);
    }
    None
}
...
mod tests {
    ...
    fn test_total() {
        ...
        //     0
        // );
        assert_eq!(get_answer(include_str!("../assets/day10_input.txt")), 98);
    }
}
