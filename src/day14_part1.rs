...
fn get_md5_string(md5_cache: &mut HashMap<usize, String>, input: &str, index: usize) -> String {
    ...
    if let Some(md5) = md5_cache.get(&index) {
        ...
        md5_str = format!("{:?}", md5::compute(str));
...
mod tests {
    ...
    fn test_first_demo_triplet() {
        let md5 = get_md5_string(&mut HashMap::new(), "abc", 18);
        assert_eq!(find_triplet(md5), Some('8'));
        for i in (18 + 1)..(18 + 1000) {
            let next_md5 = get_md5_string(&mut HashMap::new(), "abc", i);
            assert!(!contains_five_chars(next_md5, '8'));
        }
    }

    #[test]
    fn test_second_demo_triplet() {
        let md5 = get_md5_string(&mut HashMap::new(), "abc", 39);
    ...
    fn test_last_demo_triplets() {
        let md5 = get_md5_string(&mut HashMap::new(), "abc", 92);
        assert_eq!(find_triplet(md5), Some('9'));
        let mut found = false;
    ...
    #[test]
    fn test_total() {
        assert_eq!(get_answer("abc"), 22728);
        assert_eq!(get_answer("ihaygndm"), 15035);
    }
}
